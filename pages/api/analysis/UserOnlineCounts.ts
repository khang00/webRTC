...
const getData = (start: number, duration: number) => {
  const interval = 3600;
  ...
  const data = Array.from(Array(size), (item, index) => 0).map(
    (item, index) => {
      ...
      const thisDate = start * 1000 + index * 1000 * 3600;

      return {
        x: new Date(thisDate).getHours(),
        y: random(),
      };
    }
  );
...
export default function handler(req: any, res: any) {
  if (req.method === "GET") {
    ...
    res.status(200).json(getData(start, duration));
  } else {
    res.status(405);
  }
}
