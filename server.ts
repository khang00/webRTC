...
class SignalingServer {
  ...
  async getTotalUsers() {
    return Object.keys(LOGIN_USER).length;
  }

  async getUserEachRoom() {
    return Array.from(ROOMS.keys()).map((key) => {
      return {
        room: key,
        count: ROOMS.get(key).length
      };
  ...
  mountSocketEvent() {
    this.io.on("connection", (socket) => {
      ...
      socket.on("signal", (data) => {
        io.to(this.LOGIN_USER[data.to]).emit("signal", data);
      });

      socket.on("initiate", (data) => {
        io.to(this.LOGIN_USER[data.to]).emit("initiate", data);
      });
